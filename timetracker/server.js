...
app.use((req, res, next) => {
  ...
  if (shared) {
    ...
    // локальный профиль не должен оставаться админом навсегда только
    // потому, что когда-то был им повышен. Служебный автобутстрапленный
    // профиль (service) в этом случае удаляем целиком — он и был заведён
    // только ради этого права, никаких настоящих данных сотрудника (табель,
    // график) у него нет. Профиль, у которого этот флаг не выставлен
    // (например, самые старые автопривязки — см. комментарий выше про
    // "доводим их до нужного состояния", который срабатывает только пока
    // право ещё действует, и потому никогда не долечивает уже отозванные)
    // просто понижаем обратно до обычного сотрудника, а не удаляем — так
    // сохраняются его должности/график, если они были.
    if (req.user && req.user.role === 'admin' && !perms.canBecomeTtAdmin) {
      if (req.user.service) {
        db.removeServiceAdmin(req.user.id);
        req.user = null;
      } else {
        req.user = db.setUserRole(req.user.id, 'employee');
      }
    }
  }
